...
def adjust_posture(img, yellow_threshold, gray_threshold):
    """姿态调整，检测黄色和灰色色块并发送串口数据"""
    blobs_yellw = img.find_blobs([yellow_threshold])
    blobs_graw = img.find_blobs([gray_threshold])
    if blobs_yellw and blobs_graw:
        ...
        if b1 and b2 and b1.pixels() > 10000 and b2.pixels() > 10000:
            ...
            img.draw_cross(b1.cx(), b1.cy(), color=(255, 255, 0), size=20)
            img.draw_cross(b2.cx(), b2.cy(), color=(128, 128, 128), size=20)
            in_place = abs(cy_avg - 240) <= 4
            img.draw_line(b1.cx(), b1.cy(), b2.cx(), b2.cy(), color=(0, 255, 0) if in_place else (255, 0, 0))
